...
class PolywrapClientWrapped {
  ...
  public async configureAndBuild(
    configBuilder: ClientConfigBuilder
  ): Promise<Result<true, Error>> {
    const { envs, interfaces, redirects, wrappers, packages, resolvers } =
    ...
    if (
      ...
      return ResultErr(
        Error(
          'Embedded Wrappers, packages and resolvers are not yet supported in React Native.'
        )
      );
    }
